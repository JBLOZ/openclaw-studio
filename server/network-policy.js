...
const assertPublicHostAllowed = ({ host, studioAccessToken }) => {
  const allowInsecurePublicHost = String(process.env.STUDIO_ALLOW_INSECURE_PUBLIC_HOST ?? "").trim().toLowerCase();
  if (allowInsecurePublicHost === "1" || allowInsecurePublicHost === "true") return;
